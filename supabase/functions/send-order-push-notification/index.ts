...
const getStatusConfig = (status: string) => {
  const configs: Record<string, { emoji: string; title: string; body: string }> = {
    'pending': {
      ...
      body: 'Votre commande a été enregistrée et est en attente de confirmation.',
    },
    'confirmed': {
      emoji: '✓',
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const results = await Promise.allSettled(
      subscriptions.map(async (sub) => {
        ...
        try {
          ...
          // Return info about failed subscription
          return { 
            success: false, 
    ...
    console.log(`Order push notifications sent to ${userId}: ${successCount} success, ${failedResults.length} failed`);
