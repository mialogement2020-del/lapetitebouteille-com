...
import {
  Target, Clock, Trophy, Users, TrendingUp, CheckCircle, Loader2, ChevronRight,
} from "lucide-react";
...
interface ChallengeCardProps {
  challenge: Challenge;
  participation?: { current_progress: number; is_completed: boolean; bonus_claimed: boolean; };
  onJoin?: () => void;
  isJoining?: boolean;
}
...
function ChallengeCard({ challenge, participation, onJoin, isJoining }: ChallengeCardProps) {
  ...
  function formatTimeLeft(endsAt: string): string {
    ...
    if (days > 0) return t("challenges.timeLeft", { days, hours });
    return t("challenges.timeLeftHours", { hours });
  }
  ...
  return (
    <motion.div
      ...
      className={`rounded-xl border p-5 ${isCompleted ? "bg-success/10 border-success/30" : "bg-noir-light/30 border-gold/10"}`}
    >
      <div className="flex items-start justify-between gap-4 mb-4">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ backgroundColor: challenge.badge_color || "#D4AF37" + "20" }}>
            <span className="text-lg" style={{ color: challenge.badge_color || "#D4AF37" }}>
          ...
          <div>
            ...
            <Badge variant="outline" className="text-xs border-gold/30 text-cream/60">
              {t(`challenges.typeLabels.${challenge.challenge_type}`) || challenge.challenge_type}
            </Badge>
          </div>
        </div>
        {isCompleted && (
      ...
      {hasJoined && (
        <div className="mb-4">
          <div className="flex items-center justify-between text-sm mb-2">
            <span className="text-cream/60">{t("challenges.progress")}</span>
            <span className="text-cream font-medium">{participation.current_progress} / {challenge.target_value}</span>
          </div>
      ...
      <div className="flex items-center justify-between">
        ...
        {!hasJoined && onJoin && (
          <Button size="sm" onClick={onJoin} disabled={isJoining} className="bg-primary hover:bg-primary/90 text-noir">
            {isJoining ? <Loader2 className="h-4 w-4 animate-spin" /> : <>{t("challenges.join")}<ChevronRight className="h-4 w-4 ml-1" /></>}
          </Button>
        )}
      </div>
    </motion.div>
  ...
  if (isLoading) {
    return (
      <div className="space-y-4">
        {[...Array(3)].map((_, i) => <div key={i} className="h-40 rounded-xl bg-noir-light/50 animate-pulse" />)}
      </div>
    );
  }
  ...
  if (!activeChallenges?.length) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-cream/60">{t("challenges.empty")}</p>
      </div>
    );
  }

  const participationMap = new Map(myChallenges?.map((p) => [p.challenge_id, p]) || []);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="font-display text-xl text-cream">{t("challenges.title")}</h3>
          <p className="text-sm text-cream/60">{t("challenges.subtitle")}</p>
        </div>
      </div>
