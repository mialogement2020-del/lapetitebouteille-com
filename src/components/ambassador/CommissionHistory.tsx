...
const LEVEL_LABELS = ["Direct", "Niveau 2", "Niveau 3"];
    ...
    return (
      <div className="space-y-3">
        {[...Array(5)].map((_, i) => (
          <div
            key={i}
            className="h-20 rounded-lg bg-noir-light/50 animate-pulse"
          />
        ))}
      </div>
    );
  ...
  if (commissions.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-cream/60">Aucune commission pour le moment</p>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 text-sm">
                <span className="text-cream/60">
                  {LEVEL_LABELS[commission.level - 1] || `Niveau ${commission.level}`}
                </span>
                <span className="text-cream/30">•</span>
                <span className="text-cream/50">
                  Commande: {formatCurrency(commission.order_amount)}
                </span>
              </div>
            </div>
