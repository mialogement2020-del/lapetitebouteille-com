...
import { Calculator, TrendingUp, Users, Coins, ChevronRight } from "lucide-react";
import { Slider } from "@/components/ui/slider";
import { Badge } from "@/components/ui/badge";
import { formatPrice } from "@/lib/utils";

// Commission rates (matching the database)
...
export function IncomeSimulator({ currentRank = "bronze" }: { currentRank?: string }) {
  const [params, setParams] = useState<SimulationParams>({
    directReferrals: 10,
    avgOrderValue: 25000,
    ordersPerReferralPerMonth: 1,
    level2Ratio: 2,
    level3Ratio: 1.5,
  });

  const projections = useMemo(
    () => calculateProjectedEarnings(params, currentRank),
    [params, currentRank]
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="font-display text-xl text-cream">Simulateur de Revenus</h3>
      ...
      <div className="space-y-6 bg-noir-light/30 rounded-xl p-6 border border-gold/10">
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <Badge variant="outline" className="border-gold/30 text-cream">
              {params.directReferrals}
            </Badge>
          </div>
          <Slider
            value={[params.directReferrals]}
            onValueChange={([value]) =>
              setParams((p) => ({ ...p, directReferrals: value }))
            }
            min={1}
            max={100}
            step={1}
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <Badge variant="outline" className="border-gold/30 text-cream">
              {formatPrice(params.avgOrderValue)}
            </Badge>
          </div>
          <Slider
            value={[params.avgOrderValue]}
            onValueChange={([value]) =>
              setParams((p) => ({ ...p, avgOrderValue: value }))
            }
            min={10000}
            max={100000}
            step={5000}
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <label className="text-cream/80 flex items-center gap-2">
              ...
              Commandes/mois par filleul
            </label>
            <Badge variant="outline" className="border-gold/30 text-cream">
              {params.ordersPerReferralPerMonth}
            </Badge>
          </div>
          <Slider
            value={[params.ordersPerReferralPerMonth]}
            onValueChange={([value]) =>
              setParams((p) => ({ ...p, ordersPerReferralPerMonth: value }))
            }
            min={0.5}
            max={4}
            step={0.5}
      ...
      {/* Results */}
      <motion.div
        key={JSON.stringify(projections)}
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        className="bg-gradient-to-br from-primary/20 to-primary/5 rounded-xl p-6 border border-primary/30"
      >
        <h4 className="text-cream font-medium mb-4">Revenus estimés</h4>

        <div className="space-y-3">
          {/* Level breakdown */}
          <div className="flex items-center justify-between text-sm">
            <span className="text-cream/70">Niveau 1 (direct)</span>
            <span className="text-cream font-medium">
              {formatPrice(projections.level1Earnings)}/mois
            </span>
          </div>
          <div className="flex items-center justify-between text-sm">
            <span className="text-cream/70">Niveau 2</span>
            <span className="text-cream font-medium">
              {formatPrice(projections.level2Earnings)}/mois
            </span>
          </div>
          <div className="flex items-center justify-between text-sm">
            <span className="text-cream/70">Niveau 3</span>
            <span className="text-cream font-medium">
              {formatPrice(projections.level3Earnings)}/mois
            </span>
          </div>
          ...
          <div className="flex items-center justify-between">
            <span className="text-cream font-medium">Total mensuel</span>
            <span className="text-primary font-bold text-xl">
              {formatPrice(projections.totalMonthly)}
            </span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-cream/70 text-sm">Projection annuelle</span>
            <span className="text-success font-semibold">
              {formatPrice(projections.totalAnnual)}
            </span>
          </div>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-primary/20">
          <div className="flex items-center justify-between text-sm">
            <span className="text-cream/70">Taille du réseau estimée</span>
            <Badge className="bg-primary/20 text-primary border-0">
              ~{Math.round(projections.totalNetwork)} membres
            </Badge>
          </div>
        </div>
      </motion.div>
      ...
      <p className="text-xs text-cream/40 text-center">
        * Ces estimations sont indicatives et basées sur les taux de commission actuels.
        Les résultats réels dépendent de l'activité de votre réseau.
      </p>
    </div>
  );
}
