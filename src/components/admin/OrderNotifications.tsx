      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-gold/20">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-cream">{t("adminNotif.notifications")}</h3>
            {unreadCount > 0 && (
              <Badge variant="secondary" className="text-xs">
                {t("adminNotif.newCount", { count: unreadCount })}
              </Badge>
            )}
          </div>
        ...
        <ScrollArea className="max-h-[400px]">
          {isLoading ? (
            <div className="p-8 text-center">
              ...
              <p className="text-cream/60 text-sm">{t("adminNotif.loading")}</p>
            </div>
          ) : notifications.length === 0 ? (
            <div className="p-8 text-center">
              <Bell className="h-10 w-10 text-cream/20 mx-auto mb-3" />
              <p className="text-cream/60 text-sm">{t("adminNotif.noRecentOrders")}</p>
              <p className="text-cream/40 text-xs mt-1">
                {t("adminNotif.ordersInfo")}
              </p>
            </div>
          ) : (
        ...
        {notifications.length > 0 && (
          <div className="p-2 border-t border-gold/20">
            <p className="text-xs text-center text-cream/40">
              {t("adminNotif.footer24h", { count: notifications.length })}
            </p>
          </div>
        )}
      </PopoverContent>
